...
#functions to standardise formattings

standard_table <- function(data,format="html",caption="REMEMBER TO ADD A CAPTION",position="HOLD_position"){
  
data  %>% kable(caption=caption,format = format, booktabs = TRUE,format.args = list(decimal.mark = '.', big.mark = " ")) %>%  kable_styling(latex_options = c(position))
...
rm(profanity_alvarez,profanity_arr_bad,profanity_banned,profanity_racist,profanity_zac_anger)
```

```{r, show_source_data, eval=TRUE,echo=TRUE}
source_data %>% filter(id %in% c(1,6,33,49,54,160,914,3999)) %>% standard_table(.,format="latex",caption="Sample Data")
```
