...
if(!file.exists("twitter_classifier.R")){
  download.file("https://raw.githubusercontent.com/carlosyanez/Tweets_Catastrophe_Classification/master/twitter_classifier.R?token=AAAFP6NJLEOI5QM3YCQYTFC6ONV6U","twitter_classifier.R")
  }
...
processed_source <-  prepare_data(source_data,profanity_clean = 1) %>% mutate(original_text=text)
write.csv(processed_source,"processed_source_clean.csv")

rm(source_data,processed_source,profanity_alvarez,profanity_arr_bad,profanity_banned,profanity_racist,profanity_zac_anger)
```
...
if(download_processed==TRUE){
  ...
  if(download_clean==TRUE){
    ...
    source_URL <- 
    domains_URL <- "https://raw.githubusercontent.com/carlosyanez/Tweets_Catastrophe_Classification/master/domains_clean.csv?token=AAAFP6MJQEF5F25AJ5T2MTK6ONYIM"
    source_file <- "processed_source_clean.csv"
    domains_file <- "domains_clean.csv"
  }
  else{
    source_URL <- "https://raw.githubusercontent.com/carlosyanez/Tweets_Catastrophe_Classification/master/processed_source.csv?token=AAAFP6OIRWAD3RSZUYYRYZS6ONYSS"
    domains_URL <- "https://raw.githubusercontent.com/carlosyanez/Tweets_Catastrophe_Classification/master/domains.csv?token=AAAFP6P37PS7RE6JNGEXGA26ONYLK"
