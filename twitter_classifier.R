...
calculate_scores <- function(tokenised_training_data,
  ...
  if(hashtag_parameters[1]=="sin"){
    ...
    hashtags <- hashtags %>% mutate(score=(pos_proportion/as.numeric(hashtag_parameters[3]))^(as.numeric(hashtag_parameters[2]))*theta_nn/90)
...
fit_model <- function(score_vector,cv_number=20,method_word="glm",tuneGrid_word=1,
                      method_hashtag="glm",tuneGrid_hashtag=1,
                      method_link="glm",tuneGrid_link=1,
                      method_handle="glm",tuneGrid_handle=1,
                      method_aggregate="xgbTree",tuneGrid_aggregate=1){
   ...
   # fitting_model$split <- add_row(fitting_model$split,id=i,type="word")
  ...
  # fitting_model$split <- add_row(fitting_model$split,id=i,type="hashtag")
   ...
   # fitting_model$split <- add_row(fitting_model$split,id=i,type="handle")
  ...
  predictions <- score_vector$vector %>% left_join(as_tibble(predictions),by="id") %>%
    select(-id,-word_score,-hashtag_score,-handle_score,-link_score,-type) %>% filter(!is.na(`word=1`))
